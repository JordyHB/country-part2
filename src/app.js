...
async function fetchCurrencies(fetchedCountry) {
    ...
    try {
        // loops through all the entries we got from Object entries in case of multiple currencies
        for (let officialCurrency in Object.keys(currencies)) {
            // stores the acquired key in a variable
            let currencyKey = Object.keys(currencies)[officialCurrency]
            // adds it to the array, block notation required to call a key with a variable name
            currencyArray.push(currencies[currencyKey].name)
        }
        // returns an array with all official currencies
...
async function constructSearchResult(fetchedCountry) {
    ...
    const capitals = await fetchCapitals(fetchedCountry)
    ...
    if (capitals === null) {
        ...
        pTagTwo.innerText = `The capitals are ${capitals.slice(0, -1).join(', ')} and ${ capitals.slice(-1)},`
    } else {
        pTagTwo.innerText = `The capital is ${capitals[0]}`
    }
    ...
    searchResult.appendChild(flagWrapper)
    searchResult.appendChild(countryTitle)
